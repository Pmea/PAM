function [list_chords, list_times] = extractChords2(data_v, sr_hz, L_n, STEP_n, detune, chords_base)
...
for sliding = 1:STEP_n:ending
    ...
    for k = 1:length(chords_base)
        y_v = chords_base{k}; % column vector - database's chord chroma
    ...
    if chord ~= former_chord % Si on a changé d'accord
